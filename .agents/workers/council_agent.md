...
This is adapted from Andrej Karpathy's LLM Council. He dispatches queries to multiple models, has them peer-review each other anonymously, then a chairman produces the final answer. We do the same thing inside Claude/Antigravity using sub-agents with different thinking lenses instead of different models.
...
- `personal_brain.md` or similar identity docs
- Any `memory/` folder or dashboard state
...
3. A clear instruction: respond independently. Do not hedge. Lean fully into your assigned perspective.
...
Collect all 5 advisor responses. Anonymize them as Response A through E (randomize which advisor maps to which letter).
...
The chairman produces the final council verdict.
...
Generate a visual HTML report and save it to the workspace.
**File:** `council-report-[timestamp].html`

The report should be a single HTML file with inline CSS containing:
1. The question
2. The chairman's verdict prominently displayed
3. An agreement/disagreement visual breakdown
...
Save the complete transcript as `council-transcript-[timestamp].md` containing:
- Original question
- Framed question
- All 5 advisor responses
- All 5 peer reviews (de-anonymized)
